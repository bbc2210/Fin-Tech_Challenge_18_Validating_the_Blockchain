...
class Block:
    ...
    def hash_block(self):
        ...
        # my opinion: wow, I don't know we can do this way without getting any record object's attribute.
    ...
    # my op: I add this snippet of code, see the reason below.
    def to_dict(self):

        # my opinion: this condition is met when setup() is called
        if type(self.record) is str:
        ...
        elif type(self.record) is Record:
            return {
                'creator_id': self.creator_id,
                'prev_hash': self.prev_hash,
                'timestamp': self.timestamp,
                'nonce' : self.nonce,
                # my opinion: In case that the Block is initailzed with `None` value to paremeter `record`
                'sender': self.record.sender if self.record is not None else None,
                'receiver': self.record.receiver if self.record is not None else None,
                'amount': self.record.amount if self.record is not None else None
            }
...
def setup():
    ...
    # my op: the first record object type is `str`, instead of `Record`
...
# my opinion: I replace this snippet of code with mine because this code would generate the error message like this:
# "Could not convert '0b2fcd78ffa1e86e47ad4efba9be3883aa8958aa2e658c3e394ead64f4a31ae3' with type str: tried to convert to int"
# pychain_df = pd.DataFrame(pychain.chain)
# st.write(pychain_df)
i = 0
for elt in pychain.chain:
    df = pd.DataFrame(elt.to_dict(), index = [i] )
    st.write(df)
    i = i + 1
